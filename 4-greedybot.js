var GreedyBot = {
    ...
    makeMove: function() {
        ...
        this.interested_fruits[0] = 0;
        let fruit_found = false;
        var priority_fruit = 0;
        while (priority_fruit < this.interested_fruits.length - 1) {
            priority_fruit += 1;
            total_fruits = get_total_item_count(priority_fruit);
            if (fruit_found) {
                this.interested_fruits[priority_fruit] = 0;
            } else {
                if ((this.get_my_item_count(priority_fruit) > total_fruits / 2) || (this.get_opponent_item_count(priority_fruit) > total_fruits / 2)) {
                    this.interested_fruits[priority_fruit] = 0;
                } else {
                    this.interested_fruits[priority_fruit] = 1;
                    fruit_found = true;
                }
            }
        }
