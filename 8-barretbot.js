var BarretBot = {
        ...
        for (var i = 0; i < WIDTH; i++) {
            for (var j = 0; j < HEIGHT; j++) {
                if (this.board[i][j] > 0 && this.needed_fruits[this.board[i][j]] > 0) {
                    this.add_heat_to_side(i, j, 1 / this.needed_fruits[this.board[i][j]]);
    ...
    add_heat_to_side: function(x, y, strength) {
        
        var decay = 0.3;
        
        // Add strength (decayed) to the cardinal(s) that would move us closer to this x, y square.
        ...
        if (dx > 0) {
            this.heatmap[this.x - 1][this.y] += Math.exp(d * -decay) * strength;
        }
        if (dx < 0) {
            this.heatmap[this.x + 1][this.y] += Math.exp(d * -decay) * strength;
        }
        if (dy > 0) {
            this.heatmap[this.x][this.y - 1] += Math.exp(d * -decay) * strength;
        }
        if (dy < 0) {
            this.heatmap[this.x][this.y + 1] += Math.exp(d * -decay) * strength;
        }
